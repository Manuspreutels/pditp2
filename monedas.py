...
if DEBUG: 
	imshow(cimg, title="Posición de las monedas.")

# Usamos el rango de los círculos para obtener el valor de las monedas.
cant_10centavos = 0
cant_1peso = 0
cant_50centavos = 0
for _, _, radio_moneda in unique:
	if radio_moneda < 140:		# Estos valores podrían necesitar cambios si se tomara una imágen diferente.
		cant_10centavos += 1
	elif radio_moneda < 170:
		cant_1peso += 1
	else: cant_50centavos += 1

# Imprimimos los resultados por pantalla.
print("Cantidad de monedas: ", len(unique), "\nMonedas de 10 centavos:",\
	   cant_10centavos, "\nMonedas de 50 centavos:", cant_50centavos, "\nMonedas de 1 peso:", cant_1peso)
...
if DEBUG: 
	imshow(imggrad, title="imggrad - Pre-umbralado")
...
if DEBUG: 
	imshow(imggrad, title="imggrad - Umbralada")
...
if DEBUG: 
	imshow(imClausura, title="imClausura")
...
print("El valor de cada dados es:")
for i in range(len(valor_dados)):
	print("Dado", str(i+1) + ":", valor_dados[i])
